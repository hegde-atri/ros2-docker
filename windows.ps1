function Set-ROS2 {
    ...
    $env:ROS2_DOCKER_COMPOSE_STOP_COMMAND = "docker compose -f docker-compose.windows.yml down"
    $env:ROS2_DOCKER_SHELL_COMMAND = "docker exec -it ros2"

    Write-Host "ROS2 development env-variables are set:"
    Write-Host "VOLUME_MAPPING: $env:VOLUME_MAPPING"
    Write-Host "ROS2_DOCKER_COMPOSE_COMMAND: $env:ROS2_DOCKER_COMPOSE_COMMAND"
}
...
function Start-ROS2 {
    ...
    Write-Host "Use the command Shell-ROS2 to enter it."
}

function Enter-ROS2 {
    Write-Host "Creating ROS2 Shell"
    Invoke-Expression $env.$env:ROS2_DOCKER_SHELL_COMMAND
}
